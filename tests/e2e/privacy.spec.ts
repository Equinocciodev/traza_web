 ...
 * sin peticiones HTTP a terceros y sin envío al servidor al usar las tres vistas y el formulario.
 * Alcance: build local con analítica desactivada; el correo queda pendiente de envío en otra aplicación.
 */
...
test('usar las vistas y el formulario no crea cookies, almacenamiento ni peticiones salientes', async ({ page, context }) => {
  ...
  const mailtoRequests: string[] = [];
  const origin = new URL(test.info().project.use.baseURL as string).origin;
  page.on('request', (req) => {
    const url = req.url();
    const parsed = new URL(url);
    if (parsed.protocol === 'mailto:') {
      mailtoRequests.push(url);
      return;
    }
    if (parsed.origin !== origin) outgoing.push(url);
  ...
  const preparedMailto = await page.locator('[data-contact-mailto]').getAttribute('href');
  expect(preparedMailto).toMatch(/^mailto:/);
  for (const href of mailtoRequests) expect(href).toBe(preparedMailto);
...
test('las páginas no envían cabecera Set-Cookie ni cargan recursos de terceros', async ({ request }) => {
  for (const path of ['/', '/verificar/', '/recorrido/', '/institucional/', '/empresa/', '/privacidad/']) {
    ...
    // Los enlaces de navegación (<a>) no descargan recursos al cargar la página.
    const resourceTags = html.match(/<(?:script|img|source|video|audio|iframe|embed|object|link)\b[^>]*>/gi) ?? [];
    const refs = resourceTags.flatMap((tag) => [...tag.matchAll(/\s(?:src|href|data|poster)="(https?:\/\/[^"]+)"/g)].map((m) => m[1]!));
