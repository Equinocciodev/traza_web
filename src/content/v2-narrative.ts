...
const PRODUCT: SourceBeat[] = [
  ...
  },
  {
    ...
    title: copy('Se relacionan, no comparten identidad.', 'Linked, not given the same identity.'),
    summary: copy('Unidades, cajas y pallets conservan identificadores propios.', 'Units, cases and pallets keep their own identifiers.'),
    actor: copy('Participante que agrupa las unidades', 'Participant grouping the units'),
    change: copy('Se crean relaciones padre-hijo entre los niveles.', 'Parent-child relationships are created between levels.'),
    limit: copy('Consultar el grupo no prueba que cada unidad esté físicamente dentro.', 'Querying the group does not prove that each unit is physically inside.'),
    rows: [
      { label: copy('Pallet', 'Pallet'), detail: copy('Identidad propia; relación con la caja.', 'Own identity; linked to the case.') },
      { label: copy('Caja', 'Case'), detail: copy('Identidad propia; relación con la unidad.', 'Own identity; linked to the unit.') },
      { label: copy('Unidad', 'Unit'), detail: copy('Su identificador no cambia al agruparla.', 'Its identifier does not change when grouped.') },
    ],
    extra: [copy('Un contenedor puede constituir otro nivel con identidad propia. La consulta devuelve las asociaciones que constan en el registro, no un inventario físico comprobado.', 'A container may be another level with its own identity. A lookup returns the associations in the record, not a physically verified inventory.')],
  },
  {
    id: 'a4-disaggregation', chapter: 'journey', seconds: 10, kind: 'relations',
    title: copy('Separar también deja una huella.', 'Separating also leaves a record.'),
    summary: copy('Al desagrupar, se cierran las relaciones correspondientes.', 'When items are ungrouped, the relevant relationships are closed.'),
    actor: copy('Participante que recibe o desagrupa', 'Participant receiving or ungrouping items'),
  ...
  },
  {
    ...
    extra: [copy('Los eventos incorporan el rol que los aporta, el lugar y el momento declarado. Sin conocer los eventos esperados y la cobertura, no se puede concluir que hubo un movimiento sin registro.', 'Events include the contributing role, place and declared time. Without knowing expected events and coverage, an unrecorded movement cannot be inferred.')],
  },
  {
